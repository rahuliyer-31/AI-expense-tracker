  ...
  async ({ step }) => {
    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        //   percentageUsed,
        //   meetsThreshold: percentageUsed >= 80,
        //   lastAlertSent: budget.lastAlertSent,
        //   isNewMonthCheck: !budget.lastAlertSent || isNewMonth(new Date(budget.lastAlertSent), new Date()),
        //   willSendEmail: percentageUsed >= 80 && (!budget.lastAlertSent || isNewMonth(new Date(budget.lastAlertSent), new Date()))
        // });
        //
        if (
